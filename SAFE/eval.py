...
from MS_DETR_New.MS_DETR import extract_obj, ExtractObjConfig
import gc
...
def main(args):
	...
	if (not args.save_extract_features_in_eval) and (not args.collect_score_for_MSP) and (not args.collect_score_for_ODIN) and (not args.collect_score_for_Energy):
		if args.mlp_path:
	...
	skip_conditions = [args.draw_bb_config_key, args.store_eval_results_for_analysis, 
                  ...
                  args.save_box_size_based_on_boxes, args.collect_score_for_MSP, args.collect_score_for_ODIN, args.collect_score_for_Energy]
	# Specific task, penultimate layer features, comment the following code
	...
	### Exit condition
	if args.store_eval_results_for_analysis or args.save_extract_features_in_eval or args.save_box_size_based_on_boxes or args.collect_score_for_MSP or args.collect_score_for_ODIN or args.collect_score_for_Energy: exit()
...
def safe_forward(tracker, boxes, outputs, MS_DETR_infor, MLP, means, args, key_subkey_combined_layer_hook_names=None):
	...
	if args.osf_layers == '':
		...
		if args._save_extract_features_in_eval_file is None or args.draw_bb_config_key:
			extract_obj_config = ExtractObjConfig(
				save_box_size_based_on_boxes=args.save_box_size_based_on_boxes,
				collect_score_for_MSP=args.collect_score_for_MSP,
				collect_score_for_ODIN=args.collect_score_for_ODIN,
				collect_score_for_Energy=args.collect_score_for_Energy,
			)
			dict_input = extract_obj(
				MS_DETR_infor['outputs'], MS_DETR_infor['postprocessors'], tracker, MS_DETR_infor['h'], MS_DETR_infor['w'],
				args.test_opt_threshold_config['optimal_threshold'] if args.opt_threshold else None,
				extract_obj_config,
			)

			if args.save_box_size_based_on_boxes or args.collect_score_for_MSP or args.collect_score_for_ODIN or args.collect_score_for_Energy: return dict_input
...
def eval_dataset(dataloader, predictor, tracker, mapping_dict, postprocessors, model_utils, ood_scorer, cfg=None, args=None, layer_features_seperate_structure=None):
	...
	for batch_idx, input_im in enumerate(tqdm(dataloader)):
		...
		if not skip:
			...
			if args.collect_score_for_ODIN:
				args.odin_scores.append(ood_scores)
				continue

			if args.collect_score_for_Energy:
				args.energy_scores.append(ood_scores)
				continue
