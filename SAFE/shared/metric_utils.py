...
def fpr_and_fdr_at_recall(y_true, y_score, recall_level=recall_level_default,
                          pos_label=None, return_index=False, return_threshold=False):
    ...
    # print('threshold_idxs', threshold_idxs.shape, threshold_idxs[-1])

    # accumulate the true positives with decreasing threshold
    tps = stable_cumsum(y_true)[threshold_idxs]
    # print('tps', tps.shape, tps[-1])
    ...
    # print('last_ind', last_ind, tps[last_ind-2:last_ind+2], tps[last_ind])
    recall, fps, tps, thresholds = np.r_[recall[sl], 1], np.r_[fps[sl], 0], np.r_[tps[sl], 0], thresholds[sl]
    # print('recall, fps, tps, thresholds', len(recall), len(fps), len(tps), len(thresholds), recall[-5:], fps[-5:], tps[-5:], thresholds[-5:])

    cutoff = np.argmin(np.abs(recall - recall_level))
    # print('aaa', recall[cutoff], fps[cutoff] / (np.sum(np.logical_not(y_true))), fps[cutoff], thresholds[cutoff], fps[cutoff]/(fps[cutoff] + tps[cutoff]))
...
def print_measures(auroc, aupr, fpr, method_name='Ours', recall_level=recall_level_default, logger=None):
    print_fn = logger.info if logger is not None else print
    print_fn('\t\t\t\t' + method_name)
    print_fn('  FPR{:d} AUROC AUPR'.format(int(100*recall_level)))
    print_fn('& {:.2f} & {:.2f} & {:.2f}'.format(100*fpr, 100*auroc, 100*aupr))
    #print('FPR{:d}:\t\t\t{:.2f}'.format(int(100 * recall_level), 100 * fpr))
    #print('AUROC: \t\t\t{:.2f}'.format(100 * auroc))
    #print('AUPR:  \t\t\t{:.2f}'.format(100 * aupr))
