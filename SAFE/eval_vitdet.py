...
def main(args):
	...
	for cfg, dataloader, mapping_dict, name in tqdm(zip(cfgs, datasets, mappings, names)):
		...
		res = eval_dataset(
			...
			dset_file=dset_file,
			save_features=args.save_features ## Save features for later use, Hardcoded for now
		)
		dset_file.close()
...
@torch.no_grad()
def eval_dataset(dataloader, predictor , tracker, mapping_dict, postprocessors, model_utils, dset_file, save_features=False):
