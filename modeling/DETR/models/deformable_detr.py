...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, threshold):
        ...
        topk_indexes = torch.nonzero(prob.reshape(-1) > threshold).view(1, -1)
        ...
        if 'project_features' in list(outputs.keys()):
            out_project_features = torch.gather(out_project_features, 1, topk_boxes.unsqueeze(-1).repeat(1,1, out_project_features.shape[2]))
