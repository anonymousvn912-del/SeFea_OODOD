...
def plot_sensitivity_accuracy_diagram(sensitivity, accuracy, save_file_path, variant, correlation):
	...
	# Base layer-style configs: MS_DETR (decoder/SAFE) vs ViTDET (attn/mlp)
	LAYER_TYPE_BASES = {
		"MS_DETR": {
	...
	# One mapping covers all variants (MS_DETR*, ViTDET*, ViTDET_3k*) without listing each
	layer_base = "MS_DETR" if "MS_DETR" in variant else "ViTDET"
	LAYER_TYPES = LAYER_TYPE_BASES[layer_base]

	if "MS_DETR" in variant:
		baseline_layer = "transformer.decoder.layers.5.norm3_out"
	else:
		baseline_layer = "box_features"
...
def draw_sensitivity_accuracy_diagram(
	...
	"""
	Load sensitivity and accuracy for the given variant and ID/OOD setup,
	compute correlation, then plot and save the sensitivity–accuracy diagram.

	Supports normal, FGSM, and Gaussian-noise sensitivity via optional
	gaussian_noise_* and sensitivity_FGSM arguments.
	"""
	sensitivity, accuracy, info = collect_sensitiviy_and_accuracy(
		layer_specific_performance_file_path,
		variant,
		id_dataset_name,
		ood_dataset_name,
		...
		sensitivity_FGSM=sensitivity_FGSM,
		filter_input_value=filter_input_value,
		filter_fringe_values=filter_fringe_values,
	)

	correlation = compute_correlation_between_accuracy_and_sensitivity(
		sensitivity['mean'], accuracy['mean']
	)

	print(
		f'id/ood: {id_dataset_name}/{ood_dataset_name}, '
		f'corr: pearson: {correlation["pearson"]:.3f}'
		# f'corr: pearson/kendall: {correlation["pearson"]:.3f}/{correlation["kendall"]:.3f}'
	)
	print(f'variant: {variant}, sensitivity_additional_name: {info["sensitivity_additional_name"]}')

	# Output path: Visualization / roi_folder / mode_folder / id_ood_variant[suffix].png
	if 'IRoiWidth_3_IRoiHeight_6' in variant:
		parent_folder_name = '3_6'
	elif 'IRoiWidth_2_IRoiHeight_2' in variant:
		parent_folder_name = '2_2'
	elif 'IRoiWidth_2_IRoiHeight_4' in variant:
		parent_folder_name = '2_4'
	else:
		parent_folder_name = '1_1'

	if gaussian_noise_on_image_noise_mean is not None:
		store_folder_name = 'GaussianNoise'
	elif sensitivity_FGSM is not None:
		store_folder_name = 'FGSM'
	else:
		store_folder_name = 'Normal'

	suffix = ('_' + info['sensitivity_additional_name']) if info['sensitivity_additional_name'] else ''
	save_file_path = (
		f'./Visualization/{parent_folder_name}/{store_folder_name}/'
		f'{info["id_dataset_name"]}_{info["ood_dataset_name"]}_{info["variant"]}{suffix}.png'
	)

	plot_sensitivity_accuracy_diagram(
		sensitivity, accuracy, save_file_path, info['variant'], correlation
	)
	print('--------------------------------')
...
# Variant groups for sensitivity–accuracy diagram (single source of truth for --variant choices)
NORMAL_DIAGRAM_VARIANTS = (
	'MS_DETR',
	'MS_DETR_IRoiWidth_3_IRoiHeight_6',
	'MS_DETR_IRoiWidth_2_IRoiHeight_2',
	'ViTDET_IRoiWidth_2_IRoiHeight_4',
	'ViTDET_IRoiWidth_2_IRoiHeight_2',
	'ViTDET_3k_IRoiWidth_2_IRoiHeight_4',
	'ViTDET_3k_IRoiWidth_2_IRoiHeight_2',
)
FGSM_DIAGRAM_VARIANTS = (
	'MS_DETR_IRoiWidth_3_IRoiHeight_6_FGSM',
	'MS_DETR_IRoiWidth_2_IRoiHeight_2_FGSM',
	'ViTDET_3k_IRoiWidth_2_IRoiHeight_4_FGSM',
	'ViTDET_3k_IRoiWidth_2_IRoiHeight_2_FGSM',
)
GAUSSIAN_NOISE_DIAGRAM_VARIANTS = (
	'MS_DETR_IRoiWidth_3_IRoiHeight_6_GaussianNoise',
	'MS_DETR_IRoiWidth_2_IRoiHeight_2_GaussianNoise',
	'ViTDET_3k_IRoiWidth_2_IRoiHeight_4_GaussianNoise',
	'ViTDET_3k_IRoiWidth_2_IRoiHeight_2_GaussianNoise',
)
DIAGRAM_VARIANT_CHOICES = (
	NORMAL_DIAGRAM_VARIANTS + FGSM_DIAGRAM_VARIANTS + GAUSSIAN_NOISE_DIAGRAM_VARIANTS
)
...
def add_args():
	...
	parser.add_argument(
		'--variant',
		choices=DIAGRAM_VARIANT_CHOICES,
		default='MS_DETR',
		help='Variant',
	)
...
def _noise_config_for_dataset(id_dataset_name):
	"""Return (noise_means, noise_stds) for the given ID dataset."""
	if id_dataset_name == 'voc':
		return gaussian_noise_on_image_voc_noise_means, gaussian_noise_on_image_voc_noise_stds
	return gaussian_noise_on_image_bdd_noise_means, gaussian_noise_on_image_bdd_noise_stds


def run_sensitivity_accuracy_diagram(args):
	"""
	Run the sensitivity–accuracy diagram for the selected variant.
	Dispatches to normal, FGSM, or Gaussian-noise mode based on variant.
	"""
	...
	common_kw = {
		...
		'filter_input_value': args.filter_input_value,
	}

	if variant in NORMAL_DIAGRAM_VARIANTS:
		draw_sensitivity_accuracy_diagram(
			layer_specific_performance_file_path,
			variant,
			id_dataset_name,
			ood_dataset_name,
			filter_fringe_values=args.filter_fringe_values,
			**common_kw,
		)
		return

	if variant in FGSM_DIAGRAM_VARIANTS:
		draw_sensitivity_accuracy_diagram(
			layer_specific_performance_file_path,
			variant,
			id_dataset_name,
			ood_dataset_name,
			sensitivity_FGSM=FGSM_EPSILON,
			**common_kw,
		)
		return

	if variant in GAUSSIAN_NOISE_DIAGRAM_VARIANTS:
		noise_means, noise_stds = _noise_config_for_dataset(id_dataset_name)
		for mean, std in zip(noise_means, noise_stds):
			draw_sensitivity_accuracy_diagram(
				layer_specific_performance_file_path,
				variant,
				id_dataset_name,
				ood_dataset_name,
				gaussian_noise_on_image_noise_mean=mean,
				gaussian_noise_on_image_noise_std=std,
				**common_kw,
			)
		return

	raise ValueError(f'Variant "{variant}" has no diagram mode (normal, FGSM, or GaussianNoise).')


if __name__ == "__main__":
	args = add_args()
	run_sensitivity_accuracy_diagram(args)
