...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        # print(num_classes, num_queries, num_feature_levels, aux_loss)
        # print(with_box_refine, two_stage, mixed_selection, use_ms_detr)
    ...
    def forward(self, samples: NestedTensor):
        ...
        ### hack pass the return_aux to return the spatial_shapes
        return_aux = True
        if return_aux:
            hs, hs_o2m, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, anchors, spatial_shapes = self.transformer(srcs, masks, pos, query_embeds, return_aux=return_aux)
        else:
...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, threshold=None):
        ...
        if threshold is not None:
            results = [{'scores': s[s > threshold], 'labels': l[s > threshold], 'boxes': b[s > threshold], "logits_for_ood_eval": new_ped_logits[:,s > threshold,:]} for s, l, b in zip(scores, labels, boxes)]
        else:
            results = [{'scores': s, 'labels': l, 'boxes': b, "logits_for_ood_eval": new_ped_logits} for s, l, b in zip(scores, labels, boxes)]
        ...
        # prob = out_logits.sigmoid()
        # topk_indexes = torch.nonzero(prob.reshape(-1) > 0.1).view(1, -1) # they apply the threshold 0.1 to the prob
        ...
        # original_boxes = torch.gather(out_bbox, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4)) # torch.Size([1, 3, 4])
        # new_ped_logits = torch.gather(out_logits, 1,
        #                               topk_boxes.unsqueeze(-1).repeat(1,1, out_logits.shape[2])) # torch.Size([1, 3, 20])
        ...
        #         for s, l, b, ob in zip(scores, labels, boxes, original_boxes)] # Seam like we only need those variables for ood in SAFE, the others is for SIREN paper
...
def build(args):
    ...
    #     num_classes = 250
    num_classes = args.num_classes # VOC: 21, COCO: 91
