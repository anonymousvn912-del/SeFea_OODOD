...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, **kwargs):
        # hack implement to return the spatial_shapes
        if 'return_aux' in kwargs:
            return_aux = True
            kwargs.pop('return_aux')
        else:
            return_aux = False
        ...
        # Check input
        # pos_embeds 4 torch.Size([2, 256, 100, 134]) torch.Size([2, 256, 50, 67]) torch.Size([2, 256, 25, 34]) torch.Size([2, 256, 13, 17])
        # srcs 4 torch.Size([2, 256, 100, 134]) torch.Size([2, 256, 50, 67]) torch.Size([2, 256, 25, 34]) torch.Size([2, 256, 13, 17])
        # masks 4 torch.Size([2, 100, 134]) torch.Size([2, 50, 67]) torch.Size([2, 25, 34]) torch.Size([2, 13, 17])
        # print('pos_embeds', len(pos_embeds), pos_embeds[0].shape, pos_embeds[1].shape, pos_embeds[2].shape, pos_embeds[3].shape)
        # print('srcs', len(srcs), srcs[0].shape, srcs[1].shape, srcs[2].shape, srcs[3].shape)
        # print('masks', len(masks), masks[0].shape, masks[1].shape, masks[2].shape, masks[3].shape)
        ...
        # src_flatten, mask_flatten, lvl_pos_embed torch.Size([2, 17821, 256]) torch.Size([2, 17821]) torch.Size([2, 221, 256])
        # print('src_flatten, mask_flatten, lvl_pos_embed', src_flatten.shape, mask_flatten.shape, lvl_pos_embed.shape)
        ...
        if self.two_stage: ###
            if return_aux:
                return hs_o2o, hs_o2m, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, output_proposals.sigmoid(), spatial_shapes
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ...
            # print('aaa', ref_y.reshape(-1)[None].shape, valid_ratios[:, None, lvl, 1].shape)
            ref_y = ref_y.reshape(-1)[None] / (valid_ratios[:, None, lvl, 1] * H_)
            # print('aaa1', ref_x.reshape(-1)[None].shape, valid_ratios[:, None, lvl, 1].shape)
            ref_x = ref_x.reshape(-1)[None] / (valid_ratios[:, None, lvl, 0] * W_)
            # print('aaa2', ref_x.shape, ref_y.shape)
            ref = torch.stack((ref_x, ref_y), -1)
            # print('ref', ref.shape)
            reference_points_list.append(ref)
        reference_points = torch.cat(reference_points_list, 1)
        # print('cc', reference_points.shape)
        reference_points = reference_points[:, :, None] * valid_ratios[:, None]
        return reference_points

    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        # torch.Size([2, 17821, 256]) torch.Size([4, 2]) torch.Size([4])
        # torch.Size([2, 4, 2]) torch.Size([2, 17821, 256]) torch.Size([2, 17821])
        # print(src.shape, spatial_shapes.shape, level_start_index.shape)
        # print(valid_ratios.shape, pos.shape, padding_mask.shape)
        output = src
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # torch.Size([2, 300, 256]) torch.Size([2, 300, 256]) torch.Size([2, 300, 4, 4]) torch.Size([2, 17821, 256])
            # torch.Size([4, 2]) torch.Size([4]) torch.Size([2, 17821])
            # print('output, query_pos, reference_points_input, src', output.shape, query_pos.shape, reference_points_input.shape, src.shape)
            # print('src_spatial_shapes, src_level_start_index, src_padding_mask', src_spatial_shapes.shape, src_level_start_index.shape, src_padding_mask.shape)
            ...
            # torch.Size([2, 300, 256]) torch.Size([2, 300, 256])
            # print('output, output_o2m', output.shape, output_o2m.shape)
