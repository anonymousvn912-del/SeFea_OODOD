...
def add_args(method_name, mlp_weights_dir=None, siren_weight_dir=None):
	assert method_name in ['MLP', 'vMF', 'Mahalanobis'], \
		"method_name must be one of: 'MLP', 'vMF', 'Mahalanobis'"
	...
	if method_name == 'MLP':
		...
		parser.add_argument('--test-ood-datasets', nargs='+', default=['coco', 'openimages'],
						help='OOD test datasets to evaluate combined scores on')
		parser.add_argument('--use-sensitivity', action='store_true',
					help='Use sensitivity pickle to rank layers and run top-k from 1 to L')
		parser.add_argument('--sensitivity-pickle-path', type=str,
						default=os.path.join(os.path.dirname(__file__), 'AUROC_FPR95_Results', 'layer_specific_performance_v80.pkl'),
						help='Path to layer_specific_performance pickle for sensitivity')
		parser.add_argument('--sensitivity-method-key', type=str, default='',
						help='Method key in sensitivity pickle')
		parser.add_argument('--sensitivity-dataset-key', type=str, default=None,
						help='Dataset key in sensitivity pickle (default: VOC for voc, BDD for bdd)')
		parser.add_argument('--sensitivity-mode', choices=['regression', 'concatenate'], default='regression',
						help='Cross-layer combination: regression on per-layer scores, or concatenate features + PCA + SIREN')
		parser.add_argument('--pca-dim', type=int, default=None,
						help='Target dimensionality for PCA when using --sensitivity-mode concatenate (None = no PCA)')
		parser.add_argument('--concat-max-k', type=int, default=None,
						help='Max top-k layers to evaluate for concatenate+PCA sensitivity mode (default: all)')
	elif method_name == 'Mahalanobis':
		parser.add_argument('--test-ood-datasets', nargs='+', default=['coco', 'openimages'],
						help='OOD test datasets to evaluate on (default: both)')
		parser.add_argument('--layer-set', type=str, default='all',
						help='Predefined layer set name (see PREDEFINED_LAYER_SETS in mahalanobis.py). Use "all" for discovered layers excluding SAFE_features and _in.')
		parser.add_argument('--use-sensitivity', action='store_true',
						help='Use sensitivity pickle to rank layers and run top-k from 1 to L; load "all" gaussian params and precomputed train scores.')
		parser.add_argument('--sensitivity-pickle-path', type=str,
						default=os.path.join(os.path.dirname(__file__), 'AUROC_FPR95_Results', 'layer_specific_performance_v80.pkl'),
						help='Path to layer_specific_performance pickle for --use-sensitivity')
		parser.add_argument('--sensitivity-method-key', type=str,
						default='',
						help='Method key in sensitivity pickle')
		parser.add_argument('--sensitivity-dataset-key', type=str, default=None,
						help='Dataset key in sensitivity pickle (default: VOC for voc, BDD for bdd)')
