...
def add_safe_args(arg_parser):
	...
	arg_parser.add_argument('--collect-score-for-ODIN', action='store_true', help='collect the score for ODIN baseline (temperature scaling, T=1000)')
	arg_parser.add_argument('--collect-score-for-Energy', action='store_true', help='collect the score for Energy baseline (T=1, score = T*logsumexp(logits/T))')
...
if __name__ == "__main__":
	...
	assert my_utils.assert_at_most_one_true([args.cal_opt_threshold, args.draw_bb_config_key, args.store_eval_results_for_analysis, 
										  ...
										  args.collect_score_for_MSP, args.collect_score_for_ODIN, args.collect_score_for_Energy, args.gaussian_noise_on_image])
	...
	if args.cal_opt_threshold or args.draw_bb_config_key or args.store_eval_results_for_analysis or args.save_extract_features_in_eval or args.collect_score_for_MSP or args.collect_score_for_ODIN or args.collect_score_for_Energy:
		assert args.task.lower() in ['eval', 'eval_vitdet']
	...
	if args.collect_score_for_MSP:
		assert args.variant in ['MS_DETR', 'ViTDet']
		if args.variant == 'MS_DETR':
	...
	if args.collect_score_for_ODIN:
		...
		if args.variant == 'MS_DETR':
			assert args.osf_layers == 'layer_features_seperate'
		args.odin_scores_standard_file_name = f'odin_scores_{args.tdset}_{args.variant}.pkl'

	if args.collect_score_for_Energy:
		assert args.variant in ['MS_DETR', 'ViTDet']
		if args.variant == 'MS_DETR':
			assert args.osf_layers == 'layer_features_seperate'
		args.energy_scores_standard_file_name = f'energy_scores_{args.tdset}_{args.variant}.pkl'
