...
def main(
	...
	print(f"{min_allowed_score} MIN ALLOWED SCORE")
	#min_allowed_score = 0.7
	...
	with torch.no_grad():
		try:
			...
			# 		"preprocessed_predicted_instances_od2d_{}.pth".format(min_allowed_score)),
			# 	map_location=device)
			#print("I FOUND THE FILE ------------------------------------")
		# Process predictions
		except FileNotFoundError:
			#print("I COULDNT FIND THE FILE ------------------------------------")
		...
		#print("FINAL SAVING")
		#print(torch.stack(predicted_logistic_score).unique())
		with open(dictionary_file_name, "wb") as pickle_file:
			pickle.dump(false_positives_reg_analysis, pickle_file)

def main_fileless(args, cfg, modifier, min_allowed_score, only_logistic_score=False):
	...
	# Get matched results by generating them
	with torch.no_grad():
		...
		with open(prediction_file_name, 'r') as file:
			predicted_instances = json.load(file)
		
		print('min_allowed_score', min_allowed_score) ###
		print('predicted_instances', type(predicted_instances), len(predicted_instances)) ###
		preprocessed_predicted_instances = eval_predictions_preprocess(predicted_instances, min_allowed_score=min_allowed_score, 
                                                                 is_odd=True, only_logistic_score=only_logistic_score, model_name=args.variant)
		print('Time taken for eval_predictions_preprocess:', time.time() - start_time)
		...
		if only_logistic_score:
			...
			print('Time taken for itertools.chain.from_iterable:', time.time() - start_time)
			
			false_positives_reg_analysis = {}
			false_positives_reg_analysis.update({'logistic_score': predicted_logistic_score})
			return false_positives_reg_analysis

		
		print('preprocessed_predicted_instances[predicted_boxes]', type(preprocessed_predicted_instances['predicted_boxes']), len(preprocessed_predicted_instances['predicted_boxes']))
		print('preprocessed_predicted_instances[predicted_logistic_score]', type(preprocessed_predicted_instances['predicted_logistic_score']), len(preprocessed_predicted_instances['predicted_logistic_score']))
		predicted_boxes = preprocessed_predicted_instances['predicted_boxes']
		predicted_cov_mats = preprocessed_predicted_instances['predicted_covar_mats']
		predicted_cls_probs = preprocessed_predicted_instances['predicted_cls_probs']
		predicted_inter_feat = preprocessed_predicted_instances['predicted_inter_feat']
		predicted_logistic_score = None
		if 'predicted_logistic_score' in list(preprocessed_predicted_instances.keys()):
			predicted_logistic_score = preprocessed_predicted_instances['predicted_logistic_score'] ### RCNN RN50 VOS 
		...
		predicted_inter_feat = list(itertools.chain.from_iterable(
			[predicted_inter_feat[key] for key in predicted_inter_feat.keys()]))
		if predicted_logistic_score is not None:
			predicted_logistic_score = list(itertools.chain.from_iterable( ### RCNN RN50 VOS 
			[predicted_logistic_score[key] for key in predicted_logistic_score.keys()]))
  
		num_false_positives = len(predicted_boxes)
		assert num_false_positives == len(predicted_inter_feat)
		valid_idxs = torch.as_tensor([i for i in range(num_false_positives)]).to(device)
		
		predicted_boxes = torch.stack(predicted_boxes, 1).transpose(0, 1)
		predicted_cov_mats = torch.stack(predicted_cov_mats, 1).transpose(0, 1)
		predicted_cls_probs = torch.stack(predicted_cls_probs, 1).transpose(0, 1)
		...
		false_positives_cls_analysis = scoring_rules.softmax_compute_cls_scores(false_positives_dict, valid_idxs)
