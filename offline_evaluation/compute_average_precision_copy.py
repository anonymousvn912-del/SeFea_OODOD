...
def main_fileless():
	...
	with open('/home/khoadv/SAFE/SAFE_Official/a.pkl', 'rb') as f:
		args = pickle.load(f)
	...
	build_detection_test_loader(
			cfg, 
			mapper=None,
			dataset_name='voc_custom_val'
		)

	# args.test_dataset voc_custom_val
	# print('args.test_dataset', args.test_dataset)
	meta_catalog = MetadataCatalog.get('voc_custom_val')

	# Evaluate detection results
	gt_coco_api = COCO(meta_catalog.json_file)
 	# print(set(gt_coco_api.getImgIds()))
	res_coco_api = gt_coco_api.loadRes('/home/khoadv/SAFE/SAFE_Official/a.json')
	results_api = COCOeval(gt_coco_api, res_coco_api, iouType='bbox')
	...
	results_api.evaluate()
	results_api.accumulate()
	# prec = results_api.eval['precision'][0]
	# aind = [i for i, aRng in enumerate(results_api.params.areaRngLbl) if aRng == "all"]
	# mind = [i for i, mDet in enumerate(results_api.params.maxDets) if mDet == 100]
	# prec = prec[:, :, aind, mind]
	# ms = []
	...
	f1_scores = 2 * (precisions * recalls) / (precisions + recalls)
	optimal_f1_score = f1_scores.argmax(0)
	print('results_api', type(results_api))
	print('precisions, recalls', precisions.shape, recalls.shape)
	print(precisions[:10,0], recalls[:10])
	print('f1_scores', f1_scores.shape)
	scores = results_api.eval['scores'].mean(0)[:, :, 0, 2]
	print('scores', scores.shape)
	print('optimal_f1_score_i', optimal_f1_score.shape)
	optimal_score_threshold = [scores[optimal_f1_score_i, i]
							   for i, optimal_f1_score_i in enumerate(optimal_f1_score)]
	optimal_score_threshold = np.array(optimal_score_threshold)
	print('optimal_score_threshold', optimal_score_threshold.shape)
	optimal_score_threshold = optimal_score_threshold[optimal_score_threshold != 0]
	print('optimal_score_threshold', optimal_score_threshold.shape)
	optimal_score_threshold = optimal_score_threshold.mean()
	print('optimal_score_threshold', optimal_score_threshold)
