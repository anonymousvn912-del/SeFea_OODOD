...
#from modeling.detr.d2.detr.detr import Detr
...
def setup_arg_parser():
	...
	arg_parser.add_argument(
		"--inference-config",
		type=str,
		default="Inference/standard_nms.yaml",
...
def setup_config(args, random_seed=None, is_testing=False, ood=False):
	...
	args.config_file = args.config_file.replace('./configs/./configs', './configs')

	if ood: cfg.DATASETS.OOD = tuple()
	...
	# Update config with inference configurations. Only applicable for when in probabilistic inference mode.
	if args.inference_config != "":
		args.inference_config = os.path.join(
			configs_dir, args.inference_config)
		args.inference_config = args.inference_config.replace('./configs/./configs', './configs')
	...
	file_dir = core.data_dir()
	cfg['OUTPUT_DIR'] = os.path.join(core.data_dir(), dataset_name, model_name,
									 os.path.split(args.config_file)[-1][:-5], 'random_seed_0')
	...
	dataset_dir = os.path.expanduser(args.dataset_dir) # ./dataset_dir/VOC_0712_converted/
	...
	try:
		setup_all_datasets(dataset_dir, image_root_corruption_prefix=image_root_corruption_prefix, n_run=args.n_run)
	except:
		setup_all_datasets(dataset_dir, image_root_corruption_prefix=image_root_corruption_prefix)
