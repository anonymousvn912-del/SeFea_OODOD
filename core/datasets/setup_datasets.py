...
def setup_all_datasets(dataset_dir, image_root_corruption_prefix=None, n_run=None):
    """
    ...
    """
    print('setup_all_datasets', dataset_dir)
    if  "Custom_Data".lower() in dataset_dir.lower():
        assert n_run is not None
        print('&' * 100, 'n_run', n_run)
        setup_custom_dataset(dataset_dir, n_run=n_run)
    elif "VOC".lower() in dataset_dir.lower():
        setup_voc_dataset(dataset_dir)
    elif "COCO".lower() in dataset_dir.lower():
        setup_coco_dataset(dataset_dir, image_root_corruption_prefix=image_root_corruption_prefix)
        setup_coco_ood_bdd_dataset(dataset_dir)
        setup_coco_ood_dataset(dataset_dir)
    elif "OpenImages".lower() in dataset_dir.lower():
        setup_openim_odd_dataset(dataset_dir)
    elif "bdd".lower() in dataset_dir.lower():
        setup_bdd_dataset(dataset_dir)
    else:
        raise ValueError('Mismatch between dataset directory and dataset name')
...
def setup_bdd_dataset(dataset_dir):
    ...
    register_coco_instances(
        ...
        train_json_annotations,
        train_image_dir)
    MetadataCatalog.get("bdd_custom_train").thing_classes = metadata.BDD_THING_CLASSES
    MetadataCatalog.get("bdd_custom_train").thing_dataset_id_to_contiguous_id = metadata.BDD_THING_DATASET_ID_TO_CONTIGUOUS_ID
    ...
    register_coco_instances(
        ...
        test_json_annotations,
        test_image_dir)
    MetadataCatalog.get("bdd_custom_val").thing_classes = metadata.BDD_THING_CLASSES
    MetadataCatalog.get("bdd_custom_val").thing_dataset_id_to_contiguous_id = metadata.BDD_THING_DATASET_ID_TO_CONTIGUOUS_ID

def setup_voc_dataset(dataset_dir):
    train_image_dir = os.path.join(dataset_dir, 'JPEGImages')
    test_image_dir = os.path.join(dataset_dir, 'JPEGImages')

    train_json_annotations = os.path.join(dataset_dir, 'voc0712_train_all.json')
    test_json_annotations = os.path.join(dataset_dir, 'val_coco_format.json')
    ...
    register_coco_instances(
        ...
        train_json_annotations,
        train_image_dir)
    MetadataCatalog.get("voc_custom_train").thing_classes = metadata.VOC_THING_CLASSES
    MetadataCatalog.get("voc_custom_train").thing_dataset_id_to_contiguous_id = metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID
    print('metadata.VOC_THING_CLASSES', metadata.VOC_THING_CLASSES)
    print('metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID', metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID)
    ...
    register_coco_instances(
        ...
        test_json_annotations,
        test_image_dir)
    MetadataCatalog.get("voc_custom_val").thing_classes = metadata.VOC_THING_CLASSES
    MetadataCatalog.get("voc_custom_val").thing_dataset_id_to_contiguous_id = metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID
...
def setup_coco_dataset(dataset_dir, image_root_corruption_prefix=None):
    ...
    print('metadata.COCO_THING_CLASSES', metadata.COCO_THING_CLASSES)
    print('metadata.COCO_THING_DATASET_ID_TO_CONTIGUOUS_ID', metadata.COCO_THING_DATASET_ID_TO_CONTIGUOUS_ID)
...
def setup_coco_ood_dataset(dataset_dir):
    ...
    # test_json_annotations = os.path.join(
    #     dataset_dir, 'COCO-Format', 'val_coco_format.json')
    test_json_annotations = os.path.join(dataset_dir, 'annotations', 'instances_val2017_ood_rm_overlap.json')
    ...
    register_coco_instances(
        ...
        test_image_dir)
    # import ipdb; ipdb.set_trace()
    MetadataCatalog.get("coco_ood_val").thing_classes = metadata.COCO_THING_CLASSES
    MetadataCatalog.get("coco_ood_val").thing_dataset_id_to_contiguous_id = metadata.COCO_THING_DATASET_ID_TO_CONTIGUOUS_ID
...
def setup_custom_dataset(dataset_dir, n_run):
    train_image_dir = os.path.join(dataset_dir, 'JPEGImages')

    train_json_annotations = os.path.join(dataset_dir, 'custom_val.json')
    ...
    register_coco_instances(
        ...
        train_json_annotations,
        train_image_dir)
    if 'Custom_Data/ID_VOC' in dataset_dir:
        MetadataCatalog.get(f"custom_val_{n_run}").thing_classes = metadata.VOC_THING_CLASSES
        MetadataCatalog.get(
            f"custom_val_{n_run}").thing_dataset_id_to_contiguous_id = metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID
        print('metadata.VOC_THING_CLASSES', metadata.VOC_THING_CLASSES)
        print('metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID', metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID)
    else:
        MetadataCatalog.get(f"custom_val_{n_run}").thing_classes = metadata.BDD_THING_CLASSES
        MetadataCatalog.get(
            f"custom_val_{n_run}").thing_dataset_id_to_contiguous_id = metadata.BDD_THING_DATASET_ID_TO_CONTIGUOUS_ID
        print('metadata.BDD_THING_CLASSES', metadata.BDD_THING_CLASSES)
        print('metadata.BDD_THING_DATASET_ID_TO_CONTIGUOUS_ID', metadata.BDD_THING_DATASET_ID_TO_CONTIGUOUS_ID)
