...
class GeneralizedRcnnPlainPredictor(ProbabilisticPredictor):
    def __init__(self, cfg):
        super().__init__(cfg) ### RCNN RN50 VOS 

        # Define test score threshold
        self.test_score_thres = self.model.roi_heads.box_predictor.test_score_thresh  ### RCNN RN50 VOS: 0.05
        self.test_nms_thresh = self.model.roi_heads.box_predictor.test_nms_thresh  ### RCNN RN50 VOS: 0.5
        self.test_topk_per_image = self.model.roi_heads.box_predictor.test_topk_per_image  ### RCNN RN50 VOS : 100
    ...
    def generalized_rcnn_probabilistic_inference(self,
        ...
        """
        is_epistemic = ((self.mc_dropout_enabled and self.num_mc_dropout_runs > 1) or ensemble_inference) and outputs is None
        if is_epistemic:
            ...
            # input_im: (['file_name', 'height', 'width', 'image_id', 'image'])

            ####
            raw_output = dict()  ### RCNN RN50 VOS
            # print('input_im', input_im[0]['height'], input_im[0]['width'], input_im[0]['file_name'], input_im[0]['image'].shape)
            images = self.model.preprocess_image(input_im)
            # print('images', images.__getitem__(0).shape)
        ...
        # print('scores, filter_mask, box_cls 0', scores.shape, filter_mask.shape, box_cls.shape)
        scores = scores[filter_mask]
        # print('scores, filter_mask, box_cls 1', scores.shape, filter_mask.shape, box_cls.shape)
        ...
        boxes_covars = []
        return boxes, boxes_covars, scores, inter_feat, filter_inds[:,1], box_cls[filter_inds[:, 0]], det_labels

    def post_processing_standard_nms(self, input_im):  ### RCNN RN50 VOS 
        """
        ...
        """
        # print('input_img', len(input_im), [i.keys() for i in input_im], [i['image'].shape for i in input_im]) ###
