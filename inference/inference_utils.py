...
from inference.rcnn_lazy_predictor import LazyGeneralizedRcnnPlainPredictor, LazyGeneralizedRcnnPlainPredictor_Custom

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def build_lazy_predictor(cfg, cfg_model):
    # return LazyGeneralizedRcnnPlainPredictor(cfg, cfg_model)
    return LazyGeneralizedRcnnPlainPredictor_Custom(cfg, cfg_model)
...
def build_predictor(cfg):
    ...
    if cfg.MODEL.META_ARCHITECTURE == 'ProbabilisticRetinaNet':
        ...
        return GeneralizedRcnnPlainPredictor(cfg) ### RCNN RN50 VOS 
...
def general_standard_nms_postprocessing_custom(input_im,
                                        outputs,
                                        nms_threshold=0.5,
    ...
    try:
        ...
        classes_idxs, predicted_prob_vectors, det_labels, box_features, kept_rois = outputs
    except:
    ...
    keep_for_box_features = keep
    if kept_rois['get_kept_rois']:
        kept_rois['keep'] = keep
    if kept_rois['use_kept_rois']:
        keep_for_box_features = kept_rois['keep']
    box_features = box_features[keep_for_box_features]
    kept_rois['box_features'] = box_features
    if (kept_rois['get_kept_rois'] == False) and (kept_rois['use_kept_rois'] == False):
        result.box_features = box_features

    # Collect post hoc scores
    kept_rois['msp_scores'] = predicted_prob[keep].tolist()

    ODIN_TEMPERATURE = 1000.0
    odin_prob = torch.nn.functional.softmax(inter_feat[keep] / ODIN_TEMPERATURE, dim=-1)
    classes_idxs_keep = classes_idxs[keep]
    row_idx = torch.arange(classes_idxs_keep.shape[0], device=classes_idxs_keep.device)
    odin_at_pred_class = odin_prob[:, :-1][row_idx, classes_idxs_keep].tolist()
    kept_rois['odin_scores'] = odin_at_pred_class

    ENERGY_TEMPERATURE = 1.0
    inter_feat_kept = inter_feat[keep]
    kept_rois['energy_scores'] = (ENERGY_TEMPERATURE * torch.logsumexp(inter_feat_kept / ENERGY_TEMPERATURE, dim=-1)).tolist()
...
def general_standard_nms_postprocessing(input_im,
    ...
    # classes_idxs: tensor([ 4,  6,  7,  ..., 14, 15, 19], device='cuda:0') torch.Size([8082])
    # Perform nms
    keep = batched_nms(
        predicted_boxes,
        predicted_prob,
...
def probabilistic_detector_postprocess(
    ...
    # print('scale_x, scale_y', scale_x, scale_y, results.image_size[1], results.image_size[0])
    ...
    if results.has("pred_boxes_covariance"):
        # Add small value to make sure covariance matrix is well conditioned  ### RCNN RN50 VOS 
...
def instances_to_json(instances, img_id, cat_mapping_dict=None):
    ...
    # convert the VOS loading to the actual label in the annotation, which was trained the MS_DETR
    # DETR VOC: cat_mapping_dict {0: 8, 1: 9, 2: 2, 3: 10, 4: 15, 5: 11, 6: 12, 7: 3, 8: 16, 9: 4, 10: 17, 11: 5, 12: 6, 13: 13, 14: 1, 15: 18, 16: 7, 17: 19, 18: 14, 19: 20}
    # classes before [14, 8, 17]
    # print('cat_mapping_dict', cat_mapping_dict)
    # print('classes before', classes)
    classes = [cat_mapping_dict[class_i] if class_i in cat_mapping_dict.keys() else -1 for class_i in classes]
    # classes after [1, 16, 19]
