...
from inference.inference_lazy import LazyProbabilisticPredictor
from modeling.modeling_utils import covariance_output_to_cholesky, clamp_log_variance


class LazyGeneralizedRcnnPlainPredictor(LazyProbabilisticPredictor):
    def __init__(self, cfg, cfg_model):
        super().__init__(cfg, cfg_model)

        # Define test score threshold
        self.test_score_thres = self.model.roi_heads.box_predictor.test_score_thresh ## 0.6436
        ...
        print(f">>>>>>>>>>>>>>> Post-processing args: <<<<<<<<<<<<<<<<<")
        print(f'test_score_thres: {self.test_score_thres}')
        print(f'test_nms_thresh: {self.test_nms_thresh}')
        print(f'test_topk_per_image: {self.test_topk_per_image}')
    ...
    def generalized_rcnn_probabilistic_inference(self,
        ...
        if is_epistemic:
            ...
            box_features = self.model.roi_heads.box_head(box_features) ## [1000, D], D = 1024 by default
...
class LazyGeneralizedRcnnPlainPredictor_Custom(LazyGeneralizedRcnnPlainPredictor):
    
    kept_rois = {'get_kept_rois': False, 'use_kept_rois': False, 'proposals': None, 'filter_inds': None, 'keep': None, 'box_features': None, 'msp_scores': None, 'odin_scores': None, 'energy_scores': None}
    
    def generalized_rcnn_probabilistic_inference(self,input_im, outputs=None, ensemble_inference=False, outputs_list=None):
        """
        General RetinaNet probabilistic anchor-wise inference. Preliminary inference step for many post-processing
        based inference methods such as standard_nms, output_statistics, and bayes_od.
        Args:
        ...
        if is_epistemic:
            ...
            proposals_for_box_features = proposals
            if self.kept_rois['get_kept_rois']:
                self.kept_rois['proposals'] = proposals
            if self.kept_rois['use_kept_rois']:
                proposals_for_box_features = self.kept_rois['proposals']

            features = [features[f] for f in self.model.roi_heads.box_in_features]
            tmp_box_features = self.model.roi_heads.box_pooler(features, [x.proposal_boxes for x in proposals_for_box_features]) 
            tmp_box_features = self.model.roi_heads.box_head(tmp_box_features) ## [1000, D], D = 1024 by default
            ...
            raw_output.update({'box_cls': box_cls,
                               ...
                               'box_cls_var': box_cls_var,
                               'box_reg_var': box_reg_var,
                               'box_features': tmp_box_features,})
            outputs = raw_output
            ####


        proposals = outputs['proposals']
        box_cls = outputs['box_cls'] ## [N, Ncls]
        box_delta = outputs['box_delta']
        box_features = outputs['box_features']
        

        inter_feat = box_cls
        box_cls = torch.nn.functional.softmax(box_cls, dim=-1) ## N, Ncls + 1
        
        # Remove background category
        scores = box_cls[:, :-1] ## N, Ncls
        
        
        num_bbox_reg_classes = box_delta.shape[1] // 4 ## 1
        box_delta = box_delta.reshape(-1, 4)
        box_delta = box_delta.view(-1, num_bbox_reg_classes, 4) ## [N, 4*Ncls] --> [N, Ncls, 4], Ncls = 20
        filter_mask = scores > self.test_score_thres # [N, Ncls]

        filter_inds = filter_mask.nonzero(as_tuple=False) # N_chosen, 2 (2: row = box_idx in N, col_idx = cls_idx)
        
        if num_bbox_reg_classes == 1:
            box_delta = box_delta[filter_inds[:, 0], 0]
        else:
            box_delta = box_delta[filter_mask] ## N_chosen, 4

        det_labels = torch.arange(scores.shape[1], dtype=torch.long).cuda()
        det_labels = det_labels.view(1, -1).expand_as(scores) # N, Ncls
        ...
        filter_inds_for_box_features = filter_inds
        if self.kept_rois['get_kept_rois']:
            self.kept_rois['filter_inds'] = filter_inds
        if self.kept_rois['use_kept_rois']:
            filter_inds_for_box_features = self.kept_rois['filter_inds']
        box_features = box_features[filter_inds_for_box_features[:, 0]]
        ...
        return boxes, boxes_covars, scores, inter_feat, \
            filter_inds[:,1], box_cls[filter_inds[:, 0]], det_labels, box_features, self.kept_rois
    ...
    def post_processing_standard_nms(self, input_im):
        ...
        outputs, kept_rois = inference_utils.general_standard_nms_postprocessing_custom(
            input_im, outputs, self.test_nms_thresh, self.test_topk_per_image)
        
        self.kept_rois = kept_rois
