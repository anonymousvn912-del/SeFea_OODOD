...
class LazyProbabilisticPredictor(ABC):
    """
    ...
    def __init__(self, cfg, cfg_model):
        ...
        self.model = instantiate(cfg_model.model)
        self.cfg = cfg.clone()
        self.model_list = []

        # Parse config
        self.inference_mode = self.cfg.PROBABILISTIC_INFERENCE.INFERENCE_MODE ## Default: standard_nms
        self.mc_dropout_enabled = self.cfg.PROBABILISTIC_INFERENCE.MC_DROPOUT.ENABLE ## Default: False
        self.num_mc_dropout_runs = self.cfg.PROBABILISTIC_INFERENCE.MC_DROPOUT.NUM_RUNS ## Default: 1
        ...
        if self.inference_mode == 'ensembles':
            ...
            DetectionCheckpointer(self.model).load(cfg_model.train.init_checkpoint)
            # DetectionCheckpointer(
            #     self.model,
